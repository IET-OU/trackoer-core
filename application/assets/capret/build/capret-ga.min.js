...
*/
function gaTrack(urchinCode,domain,url,title,direct,event,doreturn){function rand(min,max){return min+Math.floor(Math.random()*(max-min))}var i=1E9,utmn=rand(i,9999999999),utmhid=rand(1E8,999999999),cookie=rand(1E7,99999999),random=rand(i,2147483647),today=(new Date).getTime(),win=window.location,img=new Image,enc=encodeURIComponent,urchinUrl="http://www.google-analytics.com/__utm.gif?utmwv=5.2.5&utmn="+utmn+"&utmsr=-&utmsc=-&utmul=-&utmje=0&utmfl=-&utmcs=-&utmhid="+utmhid+"&utms=5"+"&utmdt="+(title?
enc(title):"-")+(domain?"&utmhn="+domain:"")+"&utmr="+(direct?"-":enc(win))+(url?"&utmp="+enc(url):"")+(typeof event==="string"?"&utmt=event&utme=5"+event:"")+"&utmac="+urchinCode+"&utmcc="+"__utma%3D"+cookie+"."+random+"."+today+"."+today+"."+today+".2%3B%2B"+(direct?"__utmz%3D"+cookie+"."+today+".1.1.utmcsr%3D(direct)%7Cutmccn%3D(direct)%7Cutmcmd%3D(none)%3B":"__utmb%3D"+cookie+"%3B%2B__utmc%3D"+cookie+"%3B%2B__utmz%3D"+cookie+"."+today+".2.2.utmccn%3D(referral)%7Cutmcsr%3D"+win.host+"%7Cutmcct%3D"+
win.pathname+"%7Cutmcmd%3Dreferral%3B%2B__utmv%3D"+cookie+".-%3B")+"utmu=4~";if(typeof doreturn!=="undefined"&&doreturn)return urchinUrl;img.src=urchinUrl};
// Input 3
/*
 capret-ga.js: Based on capret.js
 Copyright 2012 The Open University/ License MIT/ Author N.D.Freear 2012-09-12.
 http://track.olnet.org
*/
(function(jQuery){jQuery=jQuery.noConflict();function get_data(key,mydefault){var val=jQuery("script[src*=capret-ga]").attr("data-utm_"+key);val=typeof val==="undefined"?mydefault:val;return"0"===val?false:val}var utmac=get_data("ac","UA-12345-6"),debug=get_data("debug",false),IE_comment=get_data("ie_comment",true),UA=jQuery.browser,Doc=document,DL=Doc.location,LP=oer_license_parser,log=function(s){if(typeof console==="object"&&debug)console.log(arguments.length<=1?s:arguments)};log("capret-ga");
function truncate(str,length){if(str.length>length){str=str.substring(0,length);str=str.replace(/w+$/,"");str+="..."}return str}function final_params_ga(copy_text,env){env.len=copy_text.length;env.tx=truncate(copy_text,100);env.lmod=Doc.lastModified;env.pcopy=DL.pathname+DL.search+"#!"+env.tx+"!"+"CaPReT";env.pview="/Unknown-Dest"+env.pcopy+"/"+"view";env.title=Doc.title+"/"+env.tx+"/"+env.ct;env.ecopy="(CapReT*copy*"+env.tx+")("+env.len+")";env.eview="(CapReT*view*"+env.tx+")"}function image_tag_ga(env){var img=
'<img src="'+gaTrack(env.ac,false,env.pview,env.title,env.direct,env.eview,true)+'" alt=""/>';log(img);log(env);return img}jQuery(function(){var env={ac:utmac,ct:(new Date).toUTCString(),host:DL.hostname,direct:true},license=LP.get_license(),comment=IE_comment&&UA.msie?"<\!--\n * Content copied using Internet Explorer.\n"+" * To paste rich-text cleanly use a different browser or paste into an HTML source editor on your site.\n"+" * For more help visit, http://track.olnet.org/help/capret/ie\n--\>":
"";comment=typeof IE_comment==="string"&&IE_comment.length>1&&UA.msie?"<\!--\n"+IE_comment+"\n--\>":comment;jQuery("body").clipboard({append:function(e){final_params_ga(e,env);gaTrack(env.ac,env.host,env.pcopy,env.title,env.direct);gaTrack(env.ac,env.host,env.pcopy,env.title,env.direct,env.ecopy);return comment+image_tag_ga(env)+license.license_html}})})})(jQuery);