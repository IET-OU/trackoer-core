...
photo_by_author=photo_by_user.split("<");if(logged_in_user==photo_by_author[0]){triple_array=Array(window.location.toString(),"author",logged_in_user);extract_details(triple_array);triple_array=Array()}break;default:break}}};var JSON;if(!JSON)JSON={};
...
*/
(function($){var Doc=document,jQuery=Doc.capret_no_conflict?$.noConflict():window.$,get_data=function(key,mydefault){var val=jQuery("script[src*=capret-piwik]").attr("data-piwik_"+key);val=typeof val==="undefined"?mydefault:val;return"0"===val?false:val},piwik_url=get_data("url","http://track.olnet.org/piwik"),idsite=get_data("idsite",1),source_ref=get_data("src_ref",true),IE_hack=get_data("ie_hack",false),IE_comment=get_data("ie_comment",true),record=get_data("rec",1),debug=get_data("debug",false),
UA=jQuery.browser,M=Math,enc=encodeURIComponent,J=JSON,DL=Doc.location,LP=oer_license_parser,log=function(s){if(typeof console==="object"&&debug)console.log(arguments.length<=1?s:arguments)};log("capret-piwik");function truncate(str,length){if(str.length>length){str=str.substring(0,length);str=str.replace(/w+$/,"");str+="..."}return str}function final_params_pi(copy_text,env){var tx=truncate(copy_text,100),cv="_cvar";env.idsite=idsite;env.rec=record;env.rand=M.floor(M.random()*100);env.action_name=
enc(Doc.title)+"/"+enc(" "+tx)+"/"+enc(" CaPReT");env[cv]={1:["source",DL.href],2:["length",copy_text.length],3:["text",tx],4:["modified",Doc.lastModified],5:["copyTime",(new Date).toUTCString()]};env[cv]=enc(J.stringify(env[cv]));return jQuery.param(env)}function image_tag_piwik(copy_text,env){var img='<img src="'+piwik_url+"/piwik.php?"+final_params_pi(copy_text,env)+'" alt=""/>';log(img);log(env);return img}jQuery(function(){var env={},Win=jQuery(window),urlkey=source_ref?"urlref":"url",license=
LP.get_license(),comment=IE_comment&&UA.msie?"<\!--\n * Content copied using Internet Explorer.\n"+" * To paste rich-text cleanly use a different browser or paste into an HTML source editor on your site.\n"+" * For more help visit, http://track.olnet.org/help/capret/ie\n--\>":"";comment=typeof IE_comment==="string"&&IE_comment.length>1&&UA.msie?"<\!--\n"+IE_comment+"\n--\>":comment;env[urlkey]=enc(DL.href);env.res=enc(Win.width()+"x"+Win.height());jQuery("body").clipboard({append:function(e){if(IE_hack&&
UA.msie){jQuery("body").append(image_tag_piwik(e,env));return license.license_html}return comment+image_tag_piwik(e,env)+license.license_html}})})})(jQuery);