...
class Piwik_Tracker extends Redirect_Tracker {
  ...
  * @return string URL
  */
  public function getBeaconUrl($site_id = NULL, $image, $source_host = NULL, $source_id, $source_path = NULL, $referer = NULL, $title = NULL) {

    $params = array(
	  'p' => $source_path, #Todo: parse
	  't' => $title,
    );
    $redirect_beacon_url = site_url('track/r/piwik/'. $site_id .'/'. $image .'/'. $source_host .'/'. $source_id) .'?'. http_build_query($params);

    return $redirect_beacon_url;
  }

  public function getBeaconUrl_V0($site_id = NULL, $image, $source_host = NULL, $source_path = NULL, $referer = NULL, $title = NULL, $record = 1) {
	...
	if ($this->CI->_is_debug(1)) {
	  $params->url .= '!Debug!'. $params->rand;
	  #$params->action_name .= '!Debug!'. $params->rand;

	  $params->_cvar->{'1'} = array('Via', site_url('track/'. 'Piwik') .'/..');
	  #$request->_cvar->{'2'} = array('Debug', $request->rand);
	}
	$this->CI->_debug($params);

	$params->_cvar = json_encode($params->_cvar);

	$beacon_url = $this->tracker_url . http_build_query($params);
